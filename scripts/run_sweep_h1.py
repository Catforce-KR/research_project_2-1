"""Run a guarded H1 pitch/radius smoke sweep."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "sweep_h1_smoke.yaml"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a guarded H1 pitch/radius smoke sweep."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=None,
        help="YAML config path. Defaults to configs/sweep_h1_smoke.yaml.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def normalize_config(config: dict) -> dict:
    ...
    normalized["pr_values"] = [float(value) for value in normalized["pr_values"]]
...
def print_config_summary(config_path: Path, config: dict, explicit_config: bool) -> None:
    ...
    print("H1 pitch/radius sweep runner")
...
def summarize_results(results: dict) -> None:
    print("H1 smoke sweep summary")
    for pr in sorted(results):
        row = results[pr]
        print(
            "  "
            f"P/R={float(pr):.2f}, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Failed to load H1 sweep config: {exc}")
        return 1
    ...
    from helical_propeller.sweeps import parameter_sweep_h1

    results = parameter_sweep_h1(
        radius=config["radius"],
        total_length=config["total_length"],
        n_elem=config["n_elem"],
        pr_values=config["pr_values"],
